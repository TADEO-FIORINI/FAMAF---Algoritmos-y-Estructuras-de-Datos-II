...
int main(int argc, char *argv[]) {
    ...
    int output[YEARS];
    month_t output0[YEARS];
    max_temp_anual(array,output);
    mes_max_prec_anual(array, output0);

    for (int i=0; i<YEARS; i++){
        printf("La maxima temp. de %d en Cordoba fue de %d\nLas maximas precipitaciones ocurrieron el mes %d\n\n", i+FST_YEAR, output[i], (int) output0[i]+1);
    }
    printf("\nLa minima temperatura de Cordoba entre 1980 y 2016 fue de %d\n", minima_temp_historica(array));
    return (EXIT_SUCCESS);
}
